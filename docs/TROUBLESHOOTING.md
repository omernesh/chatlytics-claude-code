...
| `403 bot_session_mismatch` from `chatlytics_send` with `requested_session: "${CHATLYTICS_SESSION}"` | v2.4.0 and earlier injected `CHATLYTICS_SESSION: "${CHATLYTICS_SESSION}"` into the MCP env; when the user hadn't defined it, Claude Code passed the literal unexpanded string to the server, which rejected it | **Fixed in v2.4.1** — the plugin no longer injects `CHATLYTICS_SESSION` in `.mcp.json`. Upgrade to v2.4.1, or on older versions: remove `CHATLYTICS_SESSION` from the MCP `env` block in your `.mcp.json` / `settings.json`. |
| HTTP 400 "...session..." from `chatlytics_send` (api_key mode) | `/api/v1/send` requires a session; the legacy dispatcher used to default it server-side | Set `CHATLYTICS_SESSION` manually in your settings `env` block (or pass `session` per call). Bot tokens are unaffected — the server pins the bot's own session. |
