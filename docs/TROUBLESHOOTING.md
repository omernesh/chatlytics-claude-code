...
| `403 bot_session_mismatch` from `chatlytics_send` | A stale `CHATLYTICS_SESSION` env var or a literal `"${CHATLYTICS_SESSION}"` placeholder is being passed as the session. | **Fixed in v2.7.6** — the plugin no longer reads `CHATLYTICS_SESSION`. Remove any `CHATLYTICS_SESSION` entry from your `.mcp.json` or `settings.json` `env` block and restart Claude Code. |
| HTTP 400 "...session..." from `chatlytics_send` (api_key mode) | `/api/v1/send` requires a session; bot tokens are server-pinned so they're unaffected | Pass `session` per-call as a parameter to `chatlytics_send` or `chatlytics_dispatch`. (`CHATLYTICS_SESSION` env var was removed in v2.7.6.) |
| Requests **time out** (~10–30s, `AbortError`) | `CHATLYTICS_API_URL` was overridden and points at an unreachable address | Unset `CHATLYTICS_API_URL` to use the default `https://node.chatlytics.ai`, or re-run `node scripts/install.mjs` (no `--url`) to restore it. |
...
| **HTTP 502**, especially during `chatlytics_poll` | Too many simultaneous long-poll consumers on the hosted API | Keep the number of concurrent Claude Code sessions low; only one session needs to long-poll at a time. |
...
| `session not found` from the API | The `session` parameter passed per-call doesn't match a real session ID | Copy the exact session ID from app.chatlytics.ai → Sessions. Bot tokens are unaffected — the server pins the session automatically. |
